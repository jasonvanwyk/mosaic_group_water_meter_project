# Contacts

**Project:** Mosaic Group Water Meter Monitoring
**Code:** MGW
**Last Updated:** 2026-02-05

---

## Client - Mosaic Group

**Company:** Mosaic Group
**Website:** https://mosaicgroup.co.za/
**Head Office:** 2 Heleza Boulevard, Sibaya Drive, eMdloti, Durban

### Site Address
**477 Anton Lembede Street, Durban** (City Life — 27 floors, 12 in scope, 576 units)

| Person | Role | Cell | Office | Email |
|--------|------|------|--------|-------|
| **Tanya Dowley** | Part Owner / Primary Contact | 072 227 0883 | 031 562 8716 | tanya@mosaicgroup.co.za |
| **Ryan** | In-house Plumber | 084 701 2780 | - | ryan@mosaicgroup.co.za |
| **Sumir** | In-house IT Technician | 084 347 5755 | - | sumir@city-life.co.za |

---

## Supplier - Precision Meters

**Company:** Precision Meters (South Africa)
**Website:** https://precisionmeters.co.za
**Address:** Unit 4B, Panther Park, 11 Berkley Road, Maitland, Cape Town 7405
**Office Phone:** 021 510 4266

| Person | Role | Cell | Email |
|--------|------|------|-------|
| **Bradley Cassani** | Smart Meter Manager | 061 864 0536 | bradley.cassani@precisionmeters.co.za |
| **Garth Le Roux** | Sales Representative | 072 469 8133 | garth@precisionmeters.co.za |

### Offices
- **Cape Town:** 021 510 4266 / info@precisionmeters.co.za
- **Johannesburg:** +27 (11) 609 6336 / salesjhb2@precisionmeters.co.za

---

## Supplier - RF Design

**Company:** RF Design
**Website:** https://www.rfdesign.co.za
**Office Phone:** 021 555 8400
**Role:** SA distributor for RAK wireless products (gateways, LoRa modules, antennas, RF components)

| Person | Role | Cell | Email |
|--------|------|------|-------|
| **Gerrie van Heerden** | Durban Contact | - | gerrie@rfdesign.co.za |

**Notes:** Existing relationship from Fairfield Dairy project. Potential supplier for RAK gateways, RAK3172 modules, antennas, cable assemblies. Also provides BOM kitting services.

---

## Contractor - Precept Systems

**Company:** Precept Systems (Pty) Ltd
**Website:** https://precept.co.za

| Person | Role | Cell | Email |
|--------|------|------|-------|
| **Jason van Wyk** | Director / Developer | 083 288 9052 | jason@precept.co.za |

### Billing Rates
| Item | Rate |
|------|------|
| Consultation | R550/hr |
| Travel (SARS 2026) | R4.76/km |

---

## Referral Chain

```
Precision Meters (Bradley Cassani)
        │
        ├── Existing relationship via Fairfield Dairy project
        │
        ▼
Precision Meters (Garth Le Roux)
        │
        ├── Offered opportunity to service Mosaic Group
        │
        ▼
Mosaic Group (Tanya Dowley)
        │
        └── Client for water meter telemetry solution
```

**Condition:** Solution must use Precision Meters smart water meters.

---

## Quick Reference

| Need | Contact | Number / Email |
|------|---------|----------------|
| Client queries | Tanya Dowley | 072 227 0883 |
| On-site IT / ERP | Sumir | 084 347 5755 |
| On-site plumbing | Ryan | 084 701 2780 |
| Meter specs/pricing | Bradley Cassani | 061 864 0536 |
| Sales/referral | Garth Le Roux | 072 469 8133 |
| RF/LoRa components | Gerrie van Heerden | gerrie@rfdesign.co.za |
